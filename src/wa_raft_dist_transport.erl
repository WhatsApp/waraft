...
-spec transport_send(
    ID :: wa_raft_transport:transport_id(), FileID :: wa_raft_transport:file_id(), State :: #sender_state{}
) ->
    {ok, NewState :: #sender_state{}}
    | {stop, Reason :: term(), NewState :: #sender_state{}}.
transport_send(ID, FileID, State) ->
    ?LOG_DEBUG(
        "wa_raft_dist_transport starting to send file ~p/~p",
        [ID, FileID],
...
transport_send_loop(ID, FileID, _Fd, eof, Peer, [], _ChunkSize, _MaxInflight, State) ->
    ...
    {ok, State};
transport_send_loop(ID, FileID, Fd, Offset, Peer, [RequestId | Chunks], ChunkSize, MaxInflight, State) when
    Offset =:= eof orelse length(Chunks) >= MaxInflight
->
    case gen_server:wait_response(RequestId, 5000) of
        ...
        {reply, {error, Reason}} ->
            ?LOG_ERROR(
                "wa_raft_dist_transport failed to send file ~p/~p due to receiver error ~p",
                [ID, FileID, Reason],
                #{domain => [whatsapp, wa_raft]}
            ),
        ...
        timeout ->
            ?LOG_ERROR(
                "wa_raft_dist_transport timed out while sending file ~p/~p",
                [ID, FileID],
                #{domain => [whatsapp, wa_raft]}
            ),
        ...
        {error, {Reason, _}} ->
            ?LOG_ERROR(
                "wa_raft_dist_transport failed to send file ~p/~p due to ~p",
                [ID, FileID, Reason],
                #{domain => [whatsapp, wa_raft]}
            ),
...
transport_send_loop(ID, FileID, Fd, Offset, Peer, Chunks, ChunkSize, MaxInflight, State) when is_integer(Offset) ->
    case prim_file:read(Fd, ChunkSize) of
        {ok, Data} ->
            ...
            transport_send_loop(
                ID, FileID, Fd, Offset + byte_size(Data), Peer, Chunks ++ [RequestId], ChunkSize, MaxInflight, State
            );
        eof ->
            transport_send_loop(ID, FileID, Fd, eof, Peer, Chunks, ChunkSize, MaxInflight, State);
        {error, Reason} ->
            ?LOG_ERROR(
                "wa_raft_dist_transport failed to read file ~p/~p due to ~p",
                [ID, FileID, Reason],
                #{domain => [whatsapp, wa_raft]}
            ),
...
when
    Request :: {chunk, wa_raft_transport:transport_id(), wa_raft_transport:file_id(), integer(), binary()}.
...
handle_call(Request, From, #receiver_state{} = State) ->
    ?LOG_NOTICE(
        "wa_raft_dist_transport got unrecognized call ~p from ~p",
        [Request, From],
        #{domain => [whatsapp, wa_raft]}
    ),
    {noreply, State}.

-spec handle_cast(Request, State :: #receiver_state{}) -> {noreply, NewState :: #receiver_state{}} when
    Request :: {complete, wa_raft_transport:transport_id(), wa_raft_transport:file_id()}.
...
handle_cast(Request, #receiver_state{} = State) ->
    ?LOG_NOTICE(
        "wa_raft_dist_transport got unrecognized cast ~p",
        [Request],
        #{domain => [whatsapp, wa_raft]}
    ),
    {noreply, State}.
...
terminate(Reason, #receiver_state{}) ->
    ?LOG_NOTICE(
        "wa_raft_dist_transport terminating due to ~p",
        [Reason],
        #{domain => [whatsapp, wa_raft]}
    ),
    ok.
...
-spec open_file(
    ID :: wa_raft_transport:transport_id(), FileID :: wa_raft_transport:file_id(), State :: #receiver_state{}
) ->
