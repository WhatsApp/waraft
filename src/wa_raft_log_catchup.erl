...
-spec start_catchup_request(Catchup :: atom(), Peer :: #raft_identity{}, FollowerLastIndex :: wa_raft_log:log_index(),
                            LeaderTerm :: wa_raft_log:log_term(), LeaderCommitIndex :: wa_raft_log:log_index(),
                            Witness :: boolean()) -> ok.
...
send_logs_impl(#raft_identity{node = PeerNode} = Peer, NextLogIndex, LeaderTerm, LeaderCommitIndex, Witness,
    ...
    Entries = case Witness of
        true ->
            {ok, RawEntries} = wa_raft_log:get(Log, NextLogIndex, LogBatchEntries, LogBatchBytes),
            wa_raft_server:stub_entries_for_witness(RawEntries);
        false ->
            {ok, RawEntries} = wa_raft_log:entries(Log, NextLogIndex, LogBatchEntries, LogBatchBytes),
            RawEntries
    end,
        ...
        _ ->
            ...
            try wa_raft_server:parse_rpc(Self, DistributionModule:call(Dest, Identifier, Command, Timeout)) of
                {LeaderTerm, _, ?APPEND_ENTRIES_RESPONSE(PrevLogIndex, true, FollowerMatchIndex, _)} ->
                    send_logs_impl(Peer, FollowerMatchIndex + 1, LeaderTerm, LeaderCommitIndex, Witness, State);
